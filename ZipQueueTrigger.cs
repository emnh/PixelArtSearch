...
namespace HvidevoldDevelopmentENK.GetPixelArt
{
    ...
    {
        public static async Task Extract(IEnumerable<IArchiveEntry> archiveEntries, string zipfile, CloudBlobContainer container, ILogger log) {                    
                    ...
                    using(MemoryStream blobMemStream = new MemoryStream()){
                        ...
                        if (isZip) {
                            using (var reader = ZipArchive.Open(blobMemStream, zipReaderOptions)) {
                                await Extract(reader.Entries, zipfile, container, log);
                            }
                        } else if (isRar) {
